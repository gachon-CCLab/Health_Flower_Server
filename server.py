...
def upload_model_to_bucket(global_model):
    ...
    bucket.upload_file(
        Filename='/Users/yangsemo/VScode/Flower_Health/%s_model.h5'%today_str,
        Key=global_model,
    )
...
def model_download():
    ...
    if '%s_model.h5'%today_str in file_list:
        model = s3_resource.download_file(bucket_name,'%s_model.h5'%today_str, '../download_model/%s_model.h5'%today_str)
    else:
...
def main() -> None:
    ...
    # ])
    if os.path.isfile('../download_model/%s_model.h5'%today_str):
        model = tf.keras.models.load_model('../download_model/%s_model.h5'%today_str)
    else:
        model = tf.keras.models.load_model('../download_model/%s_model.h5'%yesterday_str)
...
def get_eval_fn(model):
    ...
    def evaluate(
        ...
        # model save
        model.save("../%s_model.h5" % today_str)

        # s3 버킷에 global model upload
        upload_model_to_bucket("%s_model.h5" % today_str)
...
if __name__ == "__main__":
    ...
    wandb.init(entity='ccl-fl', project='server_flower', name= 'server_flower v7', dir='/Users/yangsemo/VScode/Flower_Health/wandb_server',  \
        config={"num_rounds": num_rounds,"local_epochs": local_epochs, "batch_size": batch_size,"val_steps": val_steps})
